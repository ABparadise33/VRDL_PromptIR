...
class HW4RainSnowTrainDataset(Dataset):
    def __init__(self, dataset_root, patch_size=128, augment=True):
        self.dataset_root = Path(dataset_root)
        self.degraded_dir = self.dataset_root / "train" / "degraded"
        self.clean_dir = self.dataset_root / "train" / "clean"
    ...
    def _build_samples(self):
        samples = []
        for degraded_path in list_image_files(self.degraded_dir):
            clean_path = self.clean_dir / paired_clean_name(degraded_path.name)
            if clean_path.exists():
                samples.append((degraded_path, clean_path))
        if not samples:
            raise RuntimeError(f"No training pairs found under {self.dataset_root}")
        return samples
